...
- (void)loadHead
{
    ...
    [self.head constrainToSize:CGSizeMake(200.0, 200.0)];
    [self.head centerInContainerOnAxis:NSLayoutAttributeCenterX];
    [self.head pinToSuperviewEdges:JRTViewPinTopEdge inset:30.0];
}
...
//    [self.body constrainToMinimumSize:CGSizeMake(10, 10)];
...
//    [self.body centerInContainerOnAxis:NSLayoutAttributeCenterX];
    ...
    /*
     *
     self.body x轴的左边 是点0 上部是150 也就是self.body的左上角是（0，150）
     *
     */
//    [self.body pinPointAtX:NSLayoutAttributeLeft Y:NSLayoutAttributeTop toPoint:CGPointMake(0, 350)];
    
    /*
     *
     self.body的顶部 ＝ self.head的底部 ＋ 10 也就是上下两个控件相差10像素
     *
     */
//    [self.body pinAttribute:NSLayoutAttributeTop toAttribute:NSLayoutAttributeBottom ofItem:self.head withConstant:10];
    
    [self.body pinEdges:JRTViewPinLeftEdge toSameEdgesOfView:self.head];
    
//    [self.body pinAttribute:NSLayoutAttributeTop toAttribute:NSLayoutAttributeBottom ofItem:self.head withConstant:30.0];
...
- (void)loadLeftEye
{
    ...
    [self.leftEye pinPointAtX:NSLayoutAttributeLeft Y:NSLayoutAttributeTop toPoint:CGPointMake(0, 0)];
}
...
- (void)loadRightEye
{
    ...
    [self.rightEye pinAttribute:NSLayoutAttributeLeft toAttribute:NSLayoutAttributeRight ofItem:self.leftEye withConstant:10.0];
...
- (void)loadHair
{
    ...
    [self.hair constrainToMinimumSize:CGSizeMake(0.0, 14.0)];
    [self.hair pinAttribute:NSLayoutAttributeBottom toAttribute:NSLayoutAttributeTop ofItem:self.head withConstant:-10.0];
}
